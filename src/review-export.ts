...
export function patchDigest(
	files: readonly Pick<ReviewFile, "path" | "previousPath" | "patch">[],
): string {
	const parts: string[] = [];
	const canonicalFiles = files
		.map((file) => ({
			path: digestPath(file.path),
			previousPath: file.previousPath ? digestPath(file.previousPath) : "",
			patch: file.patch,
		}))
		.sort(
			(left, right) =>
				...
				left.patch.localeCompare(right.patch),
		);
	for (const file of canonicalFiles)
		parts.push(file.path, file.previousPath, file.patch);
	return digest(parts);
}
...
function explicitTargetFields(
	...
	for (const key of TARGET_FIELDS) {
		if (value[key] !== undefined && isJsonValue(value[key]))
			fields[key] = clone(value[key]);
	}
	return fields;
}

function explicitTargetValue(
...
export function targetSignatureFromReview(
	review: Record<string, unknown>,
): string | undefined {
...
export function targetSignatureFromSnapshot(
	snapshot: Pick<ReviewSnapshot, "source" | "targetSignature">,
): string {
	if (snapshot.targetSignature) return snapshot.targetSignature;
	return object(snapshot.source)
		? (targetSignatureFromReview(snapshot.source) ?? "")
		: "";
}
...
export function normalizeReviewExport(payload: unknown): ReviewExportResult {
	const review =
		object(payload) && object(payload.review) ? payload.review : payload;
	if (!object(review))
		return {
			ok: false,
			error: {
				kind: "malformed_export",
				message: "Hunk review export must be an object.",
			},
		};

	const sessionId = rawString(
		review.sessionId ??
			review.id ??
			(object(review.session) ? review.session.id : undefined),
	);
	if (!sessionId)
		return {
			ok: false,
			error: {
				kind: "malformed_export",
				message: "Hunk review export has no session ID.",
			},
		};
	if (!Array.isArray(review.files) || review.files.length < 1)
		return {
			ok: false,
			error: {
				kind: "malformed_export",
				message: "Hunk review export has no files.",
			},
		};
	...
	for (const candidate of review.files) {
		if (!object(candidate))
			return {
				...
				error: {
					kind: "malformed_export",
					message: "Hunk review export contains an invalid file.",
				},
			};
		const path = rawString(
			candidate.path ?? candidate.filePath ?? candidate.file,
		);
		const patch = rawString(candidate.patch);
		const previousPath =
			candidate.previousPath ?? candidate.oldPath ?? candidate.from;
		...
		if (
			...
			return {
				...
				error: {
					kind: "malformed_export",
					message:
						"Every reviewed file needs path, raw patch, stats, and hunk coordinates.",
				},
			};
		if (candidate.id !== undefined && !id)
			return {
				ok: false,
				error: {
					kind: "malformed_export",
					message: "Reviewed file ID is malformed.",
				},
			};
		if (previousPath !== undefined && typeof previousPath !== "string")
			return {
				...
				error: {
					kind: "malformed_export",
					message: "Reviewed file previous path is malformed.",
				},
			};
		...
		if (fileKeys.has(key))
			return {
				...
				error: {
					kind: "malformed_export",
					message: `Hunk review export contains duplicate file path: ${path}`,
				},
			};
		fileKeys.add(key);
		files.push(
	...
	if (!Array.isArray(rawNotes))
		return {
			...
			error: {
				kind: "malformed_export",
				message:
					"Hunk review export omitted notes. Upgrade Hunk and use --include-notes.",
			},
		};
	...
	for (const candidate of rawNotes) {
		if (!object(candidate))
			return {
				...
				error: {
					kind: "malformed_export",
					message: "Hunk review export contains an invalid note.",
				},
			};
		...
		if (
			...
			(candidate.oldRange !== undefined && !oldRange) ||
			(candidate.newRange !== undefined && !newRange)
		) {
			return {
				ok: false,
				error: {
					kind: "malformed_export",
					message:
						"Human review note is missing exact body, file, or valid coordinates.",
				},
			};
		}
		const author =
			candidate.author === undefined ? undefined : clone(candidate.author);
		notes.push(
			freeze({
				...
				hunk: noteHunk(candidate),
				oldRange,
				newRange,
				author,
				timestamps: noteTimestamps(candidate),
			}),
		);
	}
	...
	return {
		...
		snapshot: freeze({
			version: 1,
			sessionId,
			source: sourceMetadata(review),
			reviewedRef,
			targetSignature,
			files: freeze(files),
			notes: freeze(notes),
			patchDigest: patch,
			reviewIdentity: reviewIdentity(patch, reviewedRef, targetSignature),
		}),
	};
}
