...
export function createGitChangesetAdapter(runner: GitCommandRunner = runGitCommand): {
	...
	return {
		async read(cwd, signal) {
			...
			if (diff.code !== 0 || diff.signal || diff.timedOut || diff.launchError) return commandFailure(diff, "diff");
			...
			if (status.code !== 0 || status.signal || status.timedOut || status.launchError) return commandFailure(status, "status");
			...
			for (const entry of statusFiles(entries)) {
				...
				if (result.timedOut || result.signal || result.launchError || (result.code !== 0 && result.code !== 1)) return commandFailure(result, "untracked diff");
