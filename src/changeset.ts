...
export function canonicalChangesetFiles(
...
export function changesetDigest(files: readonly ChangesetFile[]): string {
  const parts: string[] = [];
  for (const file of canonicalChangesetFiles(files))
...
export function isDefaultGitWorkingTreeReview(
  ...
  const source =
    ...
    !Array.isArray(snapshot.source)
      ...
      : {};
  const kind = [source.inputKind, source.kind, source.source]
    .find((value): value is string => typeof value === "string")
    ?.toLowerCase();
  if (
    kind &&
    ["raw", "patch", "staged", "jj", "sapling", "pathspec"].some((value) =>
      kind.includes(value),
    )
  )
    return false;
  if (snapshot.reviewedRef || targetSignatureFromSnapshot(snapshot))
    return false;
  for (const key of [
    "staged",
    "cached",
...
export function unknownReasonMessage(reason: ChangesetUnknownReason): string {
  switch (reason) {
    case "hunk_session_unavailable":
      return "The reviewed Hunk session is unavailable and no eligible Git fallback was captured.";
    case "hunk_export_unavailable":
      return "Hunk did not return a complete review export.";
    case "hunk_target_unsupported":
      return "This Hunk target has no safe Git fallback.";
    case "git_unavailable":
      return "Git is unavailable for the working-tree freshness check.";
    case "git_timeout":
      return "The Git freshness check timed out.";
    case "git_failed":
      return "The Git freshness check failed.";
    case "git_malformed":
      return "Git returned malformed freshness data.";
    case "git_staged_target":
      return "The review includes staged changes, which are outside the owned working-tree fallback.";
    case "git_baseline_mismatch":
      return "The captured Hunk files do not match the owned Git working-tree baseline.";
    case "missing_git_fallback":
      return "No eligible Git fallback was captured for this review.";
    case "git_root_changed":
      return "The current Git repository is different from the captured review repository.";
  }
}
