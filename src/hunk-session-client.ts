...
function failure(result: HunkCommandResult, args: string[]): HunkFailure {
	...
	if (/no active hunk session|no active hunk sessions|session.*not found|no .*session.*match|protocol-validation-failed/i.test(text)) return { kind: "session_disappeared", message: "Hunk session disappeared.", args, code: result.code ?? undefined, stderr: result.stderr };
...
export function createHunkSessionClient(runner: HunkCommandRunner = runHunkCommand): HunkSessionClient {
	return {
		async probe(cwd, config, signal) {
			...
			return id ? { ok: true, value: { sessionId: id, pid: sessionPid(result.value) } } : { ok: false, error: { kind: "session_disappeared", message: "Hunk returned no active session ID.", args } };
		},
