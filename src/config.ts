...
export function mergeConfig(base: HunkConfig, next?: HunkConfigInput): HunkConfig {
	return {
		hunk: {
...
function configFrom(value: unknown): HunkConfigInput | undefined {
	...
	if (!candidate.hunk || typeof candidate.hunk !== "object" || Array.isArray(candidate.hunk)) return undefined;
	...
	if (typeof raw.binary === "string" && raw.binary.trim()) hunk.binary = raw.binary.trim();
	return { hunk };
}

async function loadJsonFile(filePath: string): Promise<HunkConfigInput | undefined> {
	try {
...
export async function loadConfig(cwd: string): Promise<HunkConfig> {
	const globalConfig = await loadJsonFile(path.join(os.homedir(), ".pi", "agent", "hunk.json"));
