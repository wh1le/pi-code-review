...
const { normalizeReviewExport, patchDigest } = await jiti.import(path.join(repoRoot, "src/review-export.ts"), { default: false });
const { changesetDigest, isDefaultGitWorkingTreeReview } = await jiti.import(path.join(repoRoot, "src/changeset.ts"), { default: false });
...
const { createSamplingLease, handoffToSpawnedHunk, watchHunkSession } = await jiti.import(path.join(repoRoot, "src/hunk-handoff.ts"), { default: false });
...
// Complete changeset fingerprints include untracked files and conservative target checks.
{
	...
	assert.equal(currentGit.value.fingerprint.patchDigest, fallback.fingerprint.patchDigest, "unchanged working tree keeps the captured fingerprint");
	tracked = patchB;
	const changedGit = await adapter.read("/repo");
	assert.notEqual(changedGit.value.fingerprint.patchDigest, fallback.fingerprint.patchDigest, "changed working tree breaks the captured fingerprint");
	const mismatched = await adapter.captureFallback("/repo", withUntracked);
	assert.equal(mismatched.reason, "git_baseline_mismatch", "a snapshot that no longer matches the tree is stale");
...
// Sampling keeps last complete export and cancellation suppresses late loss.
{
	...
	const lease = createSamplingLease({ client, cwd: "/repo", config: DEFAULT_CONFIG, sessionId: "session-1" });
	await lease.ready();
	...
	let resolveRead;
	let losses = 0;
	const cancelled = createSamplingLease({ client: { readReview: () => new Promise((resolve) => (resolveRead = resolve)) }, cwd: "/repo", config: DEFAULT_CONFIG, sessionId: "session-1", onSessionLoss: () => losses++ });
	await new Promise((resolve) => setImmediate(resolve));
	cancelled.stop();
...
// Watching a not-owned session resolves with the last complete export once it disappears.
{
	...
	const watch = watchHunkSession({ client, cwd: "/repo", config: DEFAULT_CONFIG, sessionId: "session-1", clock });
	...
	const persistent = watchHunkSession({ client: { async readReview() { return { ok: true, value: { sessionId: "session-1", raw: review(), snapshot } }; } }, cwd: "/repo", config: DEFAULT_CONFIG, sessionId: "session-1" });
	await new Promise((resolve) => setImmediate(resolve));
	persistent.stop();
	assert.equal(await persistent.done, snapshot, "stopping forwards the last complete export without waiting for loss");
}
